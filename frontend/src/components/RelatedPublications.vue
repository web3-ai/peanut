<template>
      ...
      <div class="rounded-md relative cursor-pointer group" v-for="(publication, index) in relatedPublicationList" v-bind:key="publication.profileId+'/'+publication.publicationId" @click="goToDetail(index)">
				...
				<!-- <router-link :to="'/p/'+publication.profileId+'/'+publication.publicationId"> -->
					...
					<div class="flex flex-row items-center h-6 mt-2">
						<img :src="publication.avatar" alt="" class="rounded-full h-6 w-6">
						<span class="ml-2">{{publication.author}}</span>
						<div class="grow"></div>
